...
local function linux_read_mem()
	...
	for _, ln in ipairs(lines) do
		if ln:match("^MemTotal:") then
			total_kb = tonumber(ln:match("(%d+)"))
		end
		if ln:match("^MemAvailable:") then
			avail_kb = tonumber(ln:match("(%d+)"))
		end
...
local function mac_read_mem(cb)
	run({ "sysctl", "-n", "hw.memsize" }, function(c1, out1)
		...
		run({ "vm_stat" }, function(c2, out2)
			...
			local used_pages = pages("Pages active") + pages("Pages wired down") + pages("Pages occupied by compressor")
...
local function mac_read_net(cb)
	run({ "netstat", "-ib" }, function(code, out)
		...
		if not i_idx or not o_idx then
			return cb(nil)
		end
...
local function win_sample(cb)
	local ps = state.ps
	local script = "$ProgressPreference='SilentlyContinue';$ErrorActionPreference='SilentlyContinue';"
...
local function read_gpu_nvidia(cb)
	if not exe_exists("nvidia-smi") then
		return cb(nil)
	end
	run({
		"nvidia-smi",
...
function M.text()
	...
	if c.show.ram and m.mem_used and m.mem_total and m.mem_total > 0 then
		parts[#parts + 1] =
			make_segment(s.ram, string.format("%.1f/%.1fG", bytes_to_gib(m.mem_used), bytes_to_gib(m.mem_total)))
	end
...
local function refresh_disk_if_due()
	local t = now_ns()
	if (t - state.last_disk_at) < (state.cfg.disk_interval_ms * 1e6) then
		return
	end
	if state.pending.disk then
		return
	end
...
local function refresh_gpu_if_due()
	local t = now_ns()
	if (t - state.last_gpu_at) < (state.cfg.gpu_interval_ms * 1e6) then
		return
	end
	if state.pending.gpu then
		return
	end
...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command("ResMonOpen", function()
		M.open()
	end, {})
	vim.api.nvim_create_user_command("ResMonClose", function()
		M.close()
	end, {})
	vim.api.nvim_create_user_command("ResMonToggle", function()
		M.toggle()
	end, {})
